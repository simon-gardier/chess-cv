...
class VideoProcessing:
    def worker(frames_queue, chessboards_queue, must_resize):
        while(True):
            task = frames_queue.get()
            if task is None:
                break
            frame, int_counter = task
            if must_resize:
                frame = cv2.resize(frame, FRAME_SIZE)
            stickers = VideoProcessing.find_stickers(frame)
            if stickers:
                corners_pos = VideoProcessing.find_corners(frame, stickers)
                if corners_pos:
                    board_frame = VideoProcessing.crop_board(frame, corners_pos)
                    pieces_contours = VideoProcessing.find_pieces(board_frame)
                    if pieces_contours:
                        pos_matrix = VideoProcessing.create_pos_matrix(pieces_contours)
                        team_matrix = VideoProcessing.create_team_matrix(board_frame, pieces_contours)
                        chessboards_queue.put((int_counter, Chessboard(int_counter*FRAME_INT, stickers, corners_pos, board_frame, pieces_contours, pos_matrix, team_matrix)))
                        frames_queue.task_done()
                        continue
            chessboards_queue.put((int_counter, Chessboard(frame_number=int_counter*FRAME_INT, found=False)))
            frames_queue.task_done()
    ...
    def crop_board(frame, corners):
        ...
        destination_points = np.array([
            [0, BOARD_FRAME_SIZE[HEIGHT_INDEX] - 1],
            [0, 0],
            [BOARD_FRAME_SIZE[WIDTH_INDEX] - 1, 0],
            [BOARD_FRAME_SIZE[WIDTH_INDEX] - 1, BOARD_FRAME_SIZE[HEIGHT_INDEX] - 1]
        ], dtype=np.float32)
        perspective_matrix = cv2.getPerspectiveTransform(points, destination_points)
        squared_image = cv2.warpPerspective(frame, perspective_matrix, BOARD_FRAME_SIZE)

        return squared_image


    def create_pos_matrix(pieces_contours):
        """
        Create the position matrix based on the pieces contours
        ...
        for i in range(len(pieces_contours)):
            ...
            if moments["m00"] != 0:
                ...
                x = pieces_contours[i][:, 0, 0].mean()
                y = pieces_contours[i][:, 0, 1].mean()
    ...
    def create_team_matrix(board_frame, pieces_contours):
        """
        Returns a list with, for each contour, the team appartenance (1 or 2)
        """
        frame_hsv = cv2.cvtColor(board_frame, cv2.COLOR_BGR2HSV)
        ...
        for contour in pieces_contours:
            ...
            mean_sat = mean_hsv[1]
            mean_val = mean_hsv[2]
            contours_mean_hues.append([mean_hue, mean_sat, mean_val])
    ...
    def find_stickers(frame):
        """
        Identify the stickers on a frame. The frame is copied in HSV format. 
        Args:
            frame: A frame with no annotation and BGR colors.
        Return:
            [] if one of the stickers is not found
            [Sticker, Sticker] if the stickers are found. Pink sticker is accessible with find_stickers(frame)[PINK_INDEX], same for blue sticker.
        """
        # Init the stickers list
        stickers = [None] * 2
        stickers[BLUE_INDEX] = Sticker((FRAME_SIZE[0], FRAME_SIZE[1]/2), "blue")
        stickers[PINK_INDEX] = Sticker((0, FRAME_SIZE[1]/2), "pink")

        # Conversion to HSV and masking
        ...
        blue_contour, blue_center = VideoProcessing.find_contour(blue_mask)
        if blue_contour is None:
            return []
        stickers[BLUE_INDEX].set_pos(blue_center)
        stickers[BLUE_INDEX].set_contour(blue_contour)

        pink_contour, pink_center = VideoProcessing.find_contour(pink_mask)
        if pink_contour is None:
            return []
        stickers[PINK_INDEX].set_pos(pink_center)
        stickers[PINK_INDEX].set_contour(pink_contour)

        return stickers


    def find_corners(frame, stickers):
        """
        Find the corners using black squares detection.
        Args:
            frame: The frame to look at to find the corners.
        Returns:
            corners: The list of the outer corners.
        """
        # Find the black squares
        black_squares = VideoProcessing.find_black_squares(frame)
        ...
        for p1, p2 in [(top, bot), (top_left, bot_right), (left, right), (bot_left, top_right)]:
            cur_sum = VideoProcessing.sum_dist_line(p1, p2, centers)
            if cur_sum < min_sum:
                min_sum = cur_sum
                pair = [p1, p2]
        ...
        to_determine = VideoProcessing.get_furthest(pair[0], pair[1], points)
        # Determine the grid position of those.
        px_points, grid_pos = VideoProcessing.assign_border_corner(centers_pair, to_determine)
        ...
        ordered_centers = VideoProcessing.adjust_points(estimated_px_points, centers)
        ...
        corners_pos = VideoProcessing.assign_corners(estimated_px_points, stickers)
        return corners_pos


    def find_black_squares(frame):
        """
        ...
        squares_index_list = VideoProcessing.group_squares(black_squares)
        squares = []
        for squares_index in squares_index_list:
            ...
            cur_group = [square for i, square in enumerate(cur_group) if VideoProcessing.not_inside(square, cur_group, i)]
            if len(cur_group) == 32:
                squares = cur_group
                break
        return squares
    ...
    def group_squares(contours):
        ...
        for i in range(len(contours)):
            if i in grouped:
                ...
                VideoProcessing.search_neighbor(i, contours, cur_group, grouped)
        return groups


    def search_neighbor(cur_index, contours, cur_group, grouped):
        for i, contour in enumerate(contours):
            if i in grouped:
                continue
    ...
    def not_inside(square, squares, index):
    ...
    def assign_corners(points, stickers):
        """
        ...
        h_line_vec = np.array(stickers[PINK_INDEX].get_pos()) - np.array(stickers[BLUE_INDEX].get_pos())
        ...
        for point in points:
            ...
            if h_line >= 0 and v_line >= 0:
                coords = point.ravel()
                corners_pos[2] = (int(coords[0]), int(coords[1]))
            # Bottom left (a8)
            elif h_line >= 0 and v_line < 0:
                coords = point.ravel()
                corners_pos[1] = (int(coords[0]), int(coords[1]))
            # Bottom right (a1)
            elif h_line < 0 and v_line < 0:
                coords = point.ravel()
                corners_pos[0] = (int(coords[0]), int(coords[1]))
            # Top right (h1)
            else:
                coords = point.ravel()
                corners_pos[3] = (int(coords[0]), int(coords[1]))
        if any(corner is None for corner in corners_pos):
            return []
        return corners_pos


    def sum_dist_line(p1, p2, points, number=8):
        """
    ...
    def get_furthest(p1, p2, points, number=4):
        """
    ...
    def assign_border_corner(line, points):
        """
    ...
    def adjust_points(estimation, real):
        """
    ...
    def find_contour(color_mask):
        """
        Return the biggest contour and its center (x, y) for a given mask.
        Args:
    ...
    def find_pieces(board_frame):
        """Find the piece contours on a cropped chessboard image."""
        #on this threshold to remove the square borders.
        # Gray frame
        test_frame = cv2.cvtColor(board_frame, cv2.COLOR_BGR2GRAY)
        test_frame = cv2.medianBlur(test_frame, 5)
        # Create a black frame with the dimension of frame
        mask_list = []
        mask_list.append(np.zeros_like(test_frame))
        mask_list.append(np.zeros_like(test_frame))
        # Margin is used to avoid border slighly wrong
        margin = 5
        width = BOARD_FRAME_SIZE[0]//8
        ...
        for i in range(2):
            ...
            cv2.rectangle(mask_list[i], (0, 0), (BOARD_FRAME_SIZE), 0, 2*margin)

        # Apply the threshold on the board_frame masked
        test_list = []
        thresh_list = []
        USE_OTSU = True
        for i in range(2):
    ...
    def create_variation_list(prev_chessboard, curr_chessboard):
        """
        Creates a list of ((square_y, square_x), variation) for square that contains a piece in curr_chessboard.
        The variation is calculated on 3 factors relative to differences between prev_chessboard and curr_board.
        The list is sorted by descending order based of the variation values.
        """

        prev_board_frame = prev_chessboard.get_board_frame()
        curr_board_frame = curr_chessboard.get_board_frame()
        prev_team_matrix = prev_chessboard.get_team_matrix()
        curr_team_matrix = curr_chessboard.get_team_matrix()
        prev_pieces_contours = prev_chessboard.get_pieces_contours()
        curr_pieces_contours = curr_chessboard.get_pieces_contours()

        if prev_board_frame is None:
            return []

        # Fill a dict with for each (y, x) piece position in the board, its pixel position in (x, y)
        ...
        for i in range(len(prev_pieces_contours)):
            ...
            x /= SQUARE_SIZE
            y /= SQUARE_SIZE
            prev_contours_dict[(int(y), int(x))] = contour_pixel_position

        # Fill the variations list with values between 0 and 1 representing the variation in the squares.
        ...
        variations_list = []
        for i in range(len(curr_pieces_contours)):
            moments = cv2.moments(curr_pieces_contours[i])
            if moments["m00"] == 0:
                continue
            x = moments["m10"] / moments["m00"]
            y = moments["m01"] / moments["m00"]
            ...
            x /= SQUARE_SIZE
            y /= SQUARE_SIZE
            mask = np.zeros(curr_board_frame.shape[:2], dtype=np.uint8)
            cv2.drawContours(mask, [curr_pieces_contours[i]], contourIdx=-1, color=255, thickness=cv2.FILLED)
            curr_frame_bgr_content = curr_board_frame[mask == 255]
            prev_frame_bgr_content = prev_board_frame[mask == 255]
            ...
            prev_contour_center = prev_contours_dict.get((int(y), int(x)), None)
            if isinstance(prev_contour_center, np.ndarray):
                square_top_left = np.array([SQUARE_SIZE * int(x), SQUARE_SIZE * int(y)])
                ...
                center_variation = (np.linalg.norm(np.array(contour_relative_center) - np.array(prev_contour_relative_center)) / (np.sqrt(SQUARE_SIZE ** 2 + SQUARE_SIZE ** 2))) * 10
                center_variation = np.clip(center_variation, 0, 1)
                diff_norm += center_variation
                nb_parameters += 1
            variations_list.append((np.array([int(x), int(y)]), diff_norm / nb_parameters))
            sorted_variation_list = sorted(variations_list, key=lambda x: x[1], reverse=True)
        return sorted_variation_list
